...
const FormDialog = (props) => {
  const [imageStatus, setImageStatus] = useState(props.post.image_status);
  const [captionStatus, setCaptionStatus] = useState(props.post.caption_status);
  const [alertColor, setAlertColor] = useState("error");
  const [alertText, setAlertText] = useState("Fail");
  ...
  return (
    <React.Fragment>
      <Dialog fullWidth={true} maxWidth={"md"} open={props.open} onClose={props.handleClose}>
        ...
        <DialogContent style={{ display: "flex", justifyContent: "space-around" }}>
          <DialogContentText style={{ width: "30%" }}>
            ...
            />
            <h5>Number of image reports:&nbsp;{props.post.n_image_report || 0}</h5>
          ...
          <DialogContentText style={{ width: "60%" }}>
            ...
            />
            <h5>Number of caption reports:&nbsp;{props.post.n_caption_report || 0}</h5>
